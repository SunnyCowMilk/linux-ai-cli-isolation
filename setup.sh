...
echo -e "${BLUE}${BOLD}>>> Linux AI CLI Setup Tool v12.0${NC}"
echo "----------------------------------------------------------------"
...
EOF
    ...
    # Write to shell profile for global mode
...
EOF
    ...
    echo -e "${YELLOW}   Add to your shell profile: source ~/.claude_env${NC}"
fi
...
EOF
    ...
    echo -e "${YELLOW}   Add to your shell profile: source ~/.gemini_env${NC}"
fi
...
EOF
    ...
    echo -e "${GREEN}   ✅ Codex config written to: $CODEX_HOME/${NC}"
fi
...
if [ "$NEED_CONDA" = true ]; then
    echo -e "🔄 For isolated mode, run:"
    echo -e "   ${YELLOW}conda deactivate && conda activate $CONDA_ENV_NAME${NC}"
fi

if [ "$CLAUDE_MODE" = "global" ]; then
    echo -e "🔧 For Claude (global), add to ~/.bashrc or ~/.zshrc:"
    echo -e "   ${YELLOW}source ~/.claude_env${NC}"
fi

if [ "$GEMINI_MODE" = "global" ]; then
    echo -e "🔧 For Gemini (global), add to ~/.bashrc or ~/.zshrc:"
    echo -e "   ${YELLOW}source ~/.gemini_env${NC}"
fi

if [ "$CODEX_MODE" = "global" ]; then
    echo -e "🔧 Codex is ready to use (config in ~/.codex/)"
fi

echo ""
echo -e "📝 Enabled services:"
[ "$CLAUDE_MODE" != "disabled" ] && echo -e "   - claude"
[ "$GEMINI_MODE" != "disabled" ] && echo -e "   - gemini"
[ "$CODEX_MODE" != "disabled" ] && echo -e "   - codex"
